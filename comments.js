const data = [
  ...
  for(let i=1;i<=data.length/pageSize;i=i+1)
  {
    displaybuttons = displaybuttons+ `<button onclick="pageNumber(${i})">${i}</button>`
     
  }

  document.getElementById("button").innerHTML = displaybuttons;

  pageNumber(1);
  function pageNumber(pagenum)
  {
    let startPostion = (pagenum-1)*pageSize;//0 50
    let endpostion= startPostion+pageSize;//50 100

    let extractArray = data.slice(startPostion,endpostion);

    display(extractArray);

  }

  function display(v)
  {
    let myarray= v.map(function(t) {
